let box = document.querySelector(".rightSide")
...
let companyProduct = document.querySelector(".companyProduct");
let praceProduct = document.querySelector(".praceProduct");
...
CHECKOUT.onclick = () => {
  ...
  praceProduct.oninput = async () =>{
    valueRange.innerHTML = `Value : $${praceProduct.value}`
    try {
  ...
  companyProduct.onchange = async () =>{
    if(companyProduct.value != "all"){
      ...
      try {
        let {data} = await axios.get(`${API_URL}?company=${companyProduct.value.toUpperCase()}`)
        getData(data)
      } catch (error) {
        console.error(error);
      }
  ...
  nameProduct.onclick = async () =>{
    try {
        let {data} = await axios.get(`${API_URL}?name=${nPraduct.value}`)
        getData(data)
    } catch (error) {
        console.error(error);
    }
  }
  function PraceRange(data) {
    data = data.filter((e)=> parseFloat(e.price.replaceAll("$", "")) <= praceProduct.value)
    getData(data)
}
  btnCars.onclick = () => {
    carsina.showModal()
